...
TOOLS = [
    ...
    _tool(
        "execution_handoff",
        "Read the verified active-Codex execution handoff from a completed run. This tool never launches a recursive Codex process.",
        {"run_id": {"type": "string"}},
        ["run_id"],
    ),
]
...
def call_tool(name: str, arguments: Mapping[str, Any]) -> Any:
    ...
    if name == "run_status":
        return orchestrator.run_status(str(arguments["run_id"]))
    if name == "run_abort":
