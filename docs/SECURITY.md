...
Correct configuration is not proof of provider compatibility. Codex 0.145 custom Responses providers advertise/send `namespace` tools by default, which xAI rejected with HTTP 422 in a local probe. Removing every known tool surface allowed a direct foreground text turn, but an end-to-end spawned custom-agent test still failed with HTTP 422: xAI reported that the Codex request body did not match any accepted `ModelInput` variant. A first shell function call could also execute, but its continuation failed with a compaction-blob decode error. The shipped default does not register or select a native Grok role. Use the plugin's external xAI seats for Grok fusion, review, and xAI-hosted tools. Codex's default namespace capability is visible in the current [provider source](https://github.com/openai/codex/blob/main/codex-rs/model-provider/src/provider.rs); re-run both the spawned text test and full two-turn tool loop after either side changes.
